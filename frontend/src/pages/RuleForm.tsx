...
export function RuleForm({ rule, options, saving, onSave, onCancel }: Props) {
    ...
    <form className="form" onSubmit={submit}>
      ...
      {input.ruleType === 'FixedAmountPerTransaction' && (
        <div className="field">
          ...
          <span className="hint">Tutardan bağımsız olarak her işlem için sabit prim. İade işlemi aynı tutarı düşer.</span>
        </div>
      )}
      ...
      {input.ruleType === 'TieredRate' && (
        <div className="field">
          ...
          <div className="form-row">
            <div className="field">
              ...
              <select id="mode" value={mode} onChange={(e) => setMode(e.target.value as 'Marginal' | 'Highest')}>
                ...
                <option value="Highest">Ulaşılan dilim oranı tüm tutara</option>
              </select>
            </div>
          </div>
          <div className="tiers">
            <div className="tier-row muted" style={{ fontSize: '0.8rem' }}>
            ...
            {tiers.map((tier, i) => (
              <div className="tier-row" key={i}>
                <input type="number" min="0" step="1" value={tier.threshold} onChange={(e) => updateTier(i, { threshold: Number(e.target.value) })} />
                <input type="number" min="0" max="100" step="0.01" value={tier.rate} onChange={(e) => updateTier(i, { rate: Number(e.target.value) })} />
                <button type="button" className="small" disabled={tiers.length === 1} onClick={() => setTiers((c) => c.filter((_, j) => j !== i))}>
                  Sil
                </button>
              </div>
            ))}
          ...
          <div>
            <button type="button" className="small" onClick={() => setTiers((c) => [...c, { threshold: (c[c.length - 1]?.threshold ?? 0) + 10000, rate: (c[c.length - 1]?.rate ?? 0) + 1 }])}>
              Dilim ekle
            </button>
          </div>
